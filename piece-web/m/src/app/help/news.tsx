  ...
  return (
    <Box className={classes.root} pt={1.5} pl={2} pr={2} pb={1}>
          ...
          data.map((item, index) => {
            return (
              <React.Fragment key={index}>
                <ListItem button disableGutters className={classes.listItem} onClick={() => router.push({pathname: router.pathname + '/detail', query: {path: 'news', idx: index}}).then(() => window.scrollTo(0, 0))}>
      ...
      <Box textAlign="center">
        <Button color={"primary"} variant={"outlined"} size={"small"} onClick={() => router.push(router.pathname+'/news').then(() => window.scrollTo(0, 0))}>查看更多</Button>
      </Box>
    </Box>
  );
}
