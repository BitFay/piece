...
export default () => {
  ...
  const handleForget = async () => {
    ...
    if (auth) {
      ...
      if (res) Router.push(PATH_PREFIX + '/reset/succ');
    }
  }
  ...
  return (
    <div className={classes.root}>
      <div className={classes.content}>
        <div className={classes.title}>设置密码</div>
        <div ><Input label='请输入您的账号' fullWidth onChange={handleAccount} onFocus={handleFocus} onBlur={handleBlur} error={values.errPhoneText?.length > 0} helperText={values.errPhoneText} /></div>
